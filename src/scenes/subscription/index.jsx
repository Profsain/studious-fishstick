...
const SubscriptionManager = () => {
  ...
  const handleExtend = async (customer) => {
    ...
    if (result.isConfirmed) {
      try {
        // const updateData = { isSubscriber: true };
        const response = await fetch(
          `${apiUrl}/user/update-user/${customer._id}`,
          {
            method: "PUT",
            headers: {
  ...
  const handleDelete = async (customer) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(
          `${apiUrl}/user/delete-user/${customer._id}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
        ...
        if (response.ok) {
          setCustomerData((prevData) =>
            prevData.filter((item) => item._id !== customer._id)
          );
          Swal.fire(
          ...
          Swal.fire(
            "Error!",
            errorData.message || "Failed to delete the customer.",
            "error"
          );
        }
      } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      ...
      hide: isMobile,
      valueGetter: (params) =>
        params.row.isSubscriber ? "Subscribed" : "Free", // Display "Subscribed" or "Free"
      renderCell: (params) => (
        <Chip
          label={params.value}
          sx={{
      ...
      renderCell: (params) => (
        <ButtonGroup variant="contained">
          ...
          >
            ...
            <MenuItem
              onClick={() => {
                handleStopPlan(selectedCustomer);
                handleClose();
              }}
            >
  ...
  return (
    <Box m="20px">
      <Grid item xs={12}>
        ...
        <Typography variant="h2" fontWeight="600" color="#d66748">
          Customer Management
        </Typography>
        <Typography
          variant="subtitle2"
